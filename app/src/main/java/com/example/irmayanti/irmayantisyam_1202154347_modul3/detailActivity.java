...
public class detailActivity extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        Intent intent = getIntent(); //menangkap perintah intent untuk disimpan
        //membuat inisiasi
        ...
        //memasangkan id dengan variabel yang telah ditentukan
        TextView textTitle = findViewById(R.id.title);
        TextView textDesc = findViewById(R.id.desc);
        ImageView imageGambar = findViewById(R.id.imageview);

        //mengatur lokasi simpan untuk id yang telah dimuat dalam variabel
        textTitle.setText(title);
        textDesc.setText(descs);
        imageGambar.setImageResource(gambar);

        //menginisiasi beberapa variabel dengan id
        plus = findViewById(R.id.plus);
        minus = findViewById(R.id.minus);
        value = findViewById(R.id.txt);
        battery = findViewById(R.id.battery);

        //membuat gambar baterai dengan daya 20
        battery.setImageResource(R.drawable.ic_battery_20_black_24dp);
        value.setText(Integer.toString(counts) + "L");

    }

    public void plus(View view) { //membuat method saat button plus diklik
        counts++;
        if (value != null) {
            switch (counts) {
                //membuat mengkondisian dengan kondisi yang diperhatikan yaitu variabel counts
                case 1:
                    battery.setImageResource(R.drawable.ic_battery_20_black_24dp);
                    value.setText(Integer.toString(counts) + "L");
    ...
    public void minus(View view) { //membuat method saat button minus diklik
        counts--;
        if (value != null) {
            switch (counts) {
                case 6:
